let FluNetInteractiveReport, mapData;

d3.csv('FluNetInteractiveReport.csv')
	.then(function(FluDataset) {
		FluNetInteractiveReport = FluDataset;
		isDataReady();
	});
...
d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2.0.2/countries-10m.json')
	...
	.then(function(mapDataset) {
		mapData = topojson.feature(mapDataset, mapDataset.objects.countries).features; //convert topojson to geojson
...
function isDataReady() {
	if (mapData && FluNetInteractiveReport) {
...
function drawMap() {
	...
	projection
		// .scale(98940)
		// .rotate([00,0])
		// .center([-73.9934208,40.6875627]);
		.center([0, 80]);

	let pathGenerator = d3.geoPath(projection);
	//pathGenerator(feature); //this will return d attribute to use path. ie. "M 10 20 40..."

	let svg = d3.select('body').append('svg').attr('width', '40vw').attr('height', '80vh').style('margin','auto').style('background-color','grey');
	...
	svg.selectAll('path')
		...
		.style('fill', function(feature) {
			...
			let myRow = FluNetInteractiveReport.find(function(countryRow) {
				return countryRow['Country'] == name;
			});
			// let myRow = worldCupData.find(countryRow => countryRow['Country'] == name)
			// `${d}%` means d + '%'
			if (myRow) {
				return d3.interpolateOranges(myRow['ALL_INF'] / 180);
		...
		.on('mouseenter', function(feature) {
			...
			svg.append('text')
				.attr('x', centerPixels[0])
				.attr('y', centerPixels[1] + 10)
	...
	let g = d3.select('body').append('svg').attr('width', '90vw').attr('height', '80vh').style('margin-left','5vw').style('display','block').style('justify-content','center').append('g');

	let slider=	g.select('div#slider')
  .attr('width', 375)
  .attr('height', 200)
  .slider().axis(true).min(2000).max(2100).step(5)
